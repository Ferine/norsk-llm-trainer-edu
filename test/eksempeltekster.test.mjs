...
assert.ok(EKSEMPELTEKSTER.length >= 8, "for få tekster");
...
for (const e of EKSEMPELTEKSTER) {
  ...
  assert.ok(["fri", "ccbysa", "referat"].includes(e.lisens), `${e.id}: ukjend lisens`);
...
// NEAS-sida oppgir inga fri gjenbrukslisens. Dømet skal difor vera vårt eige
// referat med sporbar kjelde, aldri ein umerka kopi av artikkelteksten.
const surfing = EKSEMPELTEKSTER.find((e) => e.id === "sikker-surfing");
assert.ok(surfing, "manglar trygg-nett-dømet");
assert.equal(surfing.lisens, "referat");
assert.equal(surfing.url, "https://neas.no/internett/sikker-surfing/");
assert.equal(surfing.forfattar, "Språkmodell-trener");
assert.ok(!surfing.tekst.includes("Vi gjør nabolaget på nett tryggere"));
assert.ok(!surfing.tekst.includes("Jarlsberg med en hemmelig ingrediens"));
